...
function showBlur() {
    $.post('https://madInventory/showBlur');
}
...
// Autostack Quickmove
function GetSameItemSlot($toInv, $fromSlot) {
    var retval = null;
    var freeval = null;
    $.each($toInv.find(".item-slot"), function(i, slot) {
    console.log($(slot).data("item"));
    if ($(slot).data("item") != undefined){
        if ($(slot).data("item").name === $fromSlot.data("item").name) {
            if (retval === null) {
                retval = $(slot).data("item").slot;
            }
        }
    } else {
    if (freeval === null) {
...
$(document).on("mousedown", ".item-slot", function(event) {
    switch (event.which) {
        case 3:
            ...
            //toSlot = GetSameItemSlot(toInventory, $(this));
            ...
            if (ItemData.name.split("_")[0] == "weapon") {
                if (!$("#weapon-attachments").length) {
                    ...
                    $(".inv-options-list").append(
                        '<div class="inv-option-item" id="weapon-attachments"><p>Erweiterungen</p></div>'
                    );
...
function changeInventoryColor(color) {
    ...
    localStorage.setItem('madInventory-color', color);
}

const savedColor = localStorage.getItem('madInventory-color');
...
function FormatAttachmentInfo(data) {
    $.post(
        "https://madInventory/GetWeaponData",
        JSON.stringify({
            weapon: data.name,
        ...
        function(data) {
            ...
            $(".weapon-attachments-container-details").html(
                '<span style="font-weight: bold; letter-spacing: .1vh;">Seriennummer</span><br> ' +
                ClickedItemData.info.serie +
                '<br><br><span style="font-weight: bold; letter-spacing: .1vh;">Haltbarkeit - ' +
                Durability.toFixed() +
            ...
            if (data.AttachmentData !== null && data.AttachmentData !== undefined) {
                if (data.AttachmentData.length > 0) {
                    $(".weapon-attachments-title").html(
                        '<span style="font-weight: bold; letter-spacing: .1vh;">Erweiterungen</span>'
                    );
                    ...
                    $(".weapon-attachments-title").html(
                        '<span style="font-weight: bold; letter-spacing: .1vh;">Diese Waffe hat keine Erweiterungen verbaut.</span>'
                    );
                }
            } else {
                $(".weapon-attachments-title").html(
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post(
                "https://madInventory/RemoveAttachment",
                JSON.stringify({
...
$(document).on("click", ".CombineItem", function(e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post(
            "https://madInventory/combineWithAnim",
            JSON.stringify({
        ...
        $.post(
            "https://madInventory/combineItem",
            JSON.stringify({
...
function optionSwitch(
    ...
    $.post(
        "https://madInventory/SetInventoryData",
        JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            $.post("https://madInventory/PlayDropSound", JSON.stringify({}));
            $.post(
                "https://madInventory/SetInventoryData",
                JSON.stringify({
                    fromInventory: $fromInv.attr("data-inventory"),
            ...
            if (fromData.amount == $toAmount) {
                if (
                    ...
                    $.post(
                        "https://madInventory/getCombineItem",
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("https://madInventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Close = function() {
        ...
        $.post("https://madInventory/CloseInventory", JSON.stringify({}));
    ...
    Inventory.UseItem = function(data) {
        ...
        $("#itembox-action").html("<p>Benutzt 1x</p>");
   ...
   Inventory.itemBox = function(data) {
       ...
       var type = "Benutzt " + data.itemAmount + "x";
       if (data.type == "add") {
         type = "Erhalten " + data.itemAmount + "x";
       } else if (data.type == "remove") {
         type = "Entfernt " + data.itemAmount + "x";
       }
